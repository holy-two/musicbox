...
class SynthController extends abcjs.synth.SynthController {
    get _randomAccess() {
        return (ev: PointerEvent) => {
            ...
            const aside = document.querySelector("#root>aside") as HTMLDivElement;
            const background = button.classList.contains('abcjs-midi-progress-indicator') ? button.parentNode as HTMLElement : button;
            const percent = Math.min(Math.max((ev.x - aside.offsetWidth - background.offsetLeft) / background.offsetWidth, 0), 1);
